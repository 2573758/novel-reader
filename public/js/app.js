...
async function api(url,opt){opt=opt||{};var r=await fetch(url,{signal:AbortSignal.timeout(opt.timeout||15000)});if(!r.ok){var e;try{e=await r.json();}catch(x){e={error:'HTTP '+r.status}}throw new Error(e.error||'Request failed');}return r.json();}
async function fn(id){var data=await api('/api/novel/'+id);if(data.cover)data.cover='/api/proxy-image?url='+encodeURIComponent(data.cover);return data;}
async function fc(nid,cid){var ca=Storage.getCachedChapter(nid,cid);if(ca)return ca;
var b=Storage.getBook(nid),np=(b&&b.novelPath)?b.novelPath:null;
var url='/api/chapter?novelId='+encodeURIComponent(nid)+'&chapterId='+cid;
if(np&&np!==nid)url+='&np='+encodeURIComponent(np);
var data=await api(url,{timeout:20000});
Storage.cacheChapter(nid,cid,data);return data;}
...
h+='<div class="book-card" data-id="'+esc(id)+'" onclick="App.ob(\''+esc(id)+'\')"><div class="book-cover">'+(b.cover?'<img src="'+esc(b.cover)+'" alt="'+esc(b.title)+'" loading="lazy" onerror="this.outerHTML=\'\u{1F4D5}\'">':'\u{1F4D5}')+'</div><div class="book-info"><div class="book-title">'+esc(b.title)+'</div>'+(pt?'<div class="book-progress">'+pt+'</div>':'')+'</div><button class="delete-btn" onclick="event.stopPropagation();App.cd(\''+esc(id)+'\')">X</button></div>';});}
...
Storage.saveBook(id,{id:data.id,title:data.title,author:data.author,cover:data.cover,description:data.description,volumes:data.volumes,novelPath:data.novelPath,chaptersCount:flat.length,lastRead:null,chaptersCache:{}});
...
var h='<div class="detail-view"><div class="detail-hero"><button class="back-btn" onclick="App.gs()">\u2190</button><div class="detail-cover">'+(b.cover?'<img src="'+esc(b.cover)+'" alt="'+esc(b.title)+'" onerror="this.outerHTML=\'\u{1F4D5}\'">':'\u{1F4D5}')+'</div><div class="detail-title">'+esc(b.title)+'</div><div class="detail-author">'+esc(b.author)+'</div><div class="detail-actions">'+(p?'<button class="btn btn-primary" onclick="App.cr(\''+esc(id)+'\')">Continue</button>':'')+'<button class="btn '+(p?'btn-ghost':'btn-primary')+'" onclick="App.sr(\''+esc(id)+'\')">'+(p?'From start':'Read')+'</button></div></div><div class="detail-body">';
